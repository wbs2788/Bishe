...
分支时刻 $\tau$ 是决定数据集质量的关键超参数。
在本研究中，将其设定为总步数的最后 10\% 阶段，即 $\tau = \lfloor 0.1 \times T_{train} \rfloor$（例如，若总步数为 50，则在剩余 5 步时分支）。
这一选择基于以下权衡分析：
\begin{itemize}
    \item 若分支过早（$\tau \gg 0.1 T$）： 独立演化的路径过长。
    由于扩散模型的累积误差放大效应，视频的背景和动作会在多步迭代后发生显著偏离，破坏了对齐性。
    \item 若分支过晚（$\tau \approx 0$）： 留给语义修改的步数不足。
    模型已在共享阶段将目标概念的视觉特征“固化”在潜变量中，仅靠最后几步无法有效擦除。
\end{itemize}

由于Wan-2.1 模型采用流匹配（Flow Matching）架构，其生成轨迹较为平直且高效。
实验表明，该模型在生成过程的最后 10\% 阶段集中处理细粒度的语义渲染。
因此，选择 $t_{0.1}$ 是在“保持时空一致性”与“实现概念擦除”之间的最佳平衡点。
...
尽管基于潜空间分支的生成算法在机理上保证了成对视频的时空对齐性，但由于扩散模型的随机采样特性，生成的原始数据仍可能存在两类噪声：

\begin{itemize}
    \item 正样本生成失败：提示词是“钢铁侠”，但生成了普通的机器人；
    \item 负样本擦除不彻底：提示词已去除“米老鼠”，但画面中仍残留了红裤子或圆耳朵特征。
\end{itemize}

为了确保 VCE-24K 数据集的高质量与纯净度，我们构建了一套基于 CLIP（Contrastive Language-Image Pre-training）模型的自动化清洗流水线，并辅以严格的人工抽检机制。
...
基于上述评分函数，我们设计了严格的“正负双验”过滤逻辑。
只有当一对样本 $(V^+, V^-)$ 同时满足以下两个条件时，才会被纳入最终数据集：
\begin{itemize}
    \item 条件一：正样本高置信度 (Positive Confidence Check)$$S(V^+, x^+) > \delta_{high}$$物理释义： 确保正样本 $V^+$ 忠实地还原了目标概念 $x^+$。经过在验证集上的统计分布分析，我们将 $\delta_{high}$ 设定为 0.28。低于此阈值通常意味着生成质量崩塌或概念特征模糊。
    \item 条件二：负样本高纯净度 (Negative Cleanliness Check)$$S(V^-, x^+) < \delta_{low}$$物理释义： 这是清洗流程中最关键的一环。注意，我们并未计算 $V^-$ 与 $x^-$ 的相似度，而是计算 $V^-$ 与正样本提示词 $x^+$ 的相似度。如果 $S(V^-, x^+)$ 依然很高，说明负样本中发生了概念泄漏（Concept Leakage）（即虽然提示词改了，但画面没变）。我们将 $\delta_{low}$ 设定为 0.22。只有低于此阈值，才能证明目标概念在视觉上已被彻底移除。
\end{itemize}

通过这一自动化流水线，剔除了约 18\% 的原始生成数据，确保了留存数据的语义准确性。
\subsubsection{人工抽检与可靠性验证}
为了进一步验证自动化清洗流程的可靠性，我们引入了“人在回路（Human-in-the-loop）”的抽检机制。
我们采用分层抽样法，从最终数据集中随机抽取了 1,000 对视频样本（覆盖所有 20 个概念类别），并组织了 3 名标注人员进行双盲复核。
复核标准包含三个维度：
\begin{itemize}
    \item 概念存在性：$V^+$ 中是否清晰可见目标概念？
    \item 擦除彻底性：$V^-$ 中是否完全不存在目标概念的身份特征？
    \item 结构对齐性：$V^+$ 和 $V^-$ 的背景与动作是否基本一致？
\end{itemize}
...
\begin{table}[!t]
    ...
    \begin{tabularx}{\textwidth}{l Y X} % 第一列左对齐，第二列换行居中，第三列换行左对齐
        \toprule
        \textbf{评估维度} & \textbf{通过率} & \textbf{典型失败案例分析} \\
        \midrule
        正样本质量 & 98.2\% & 少数样本因肢体生成畸变被判为失败。 \\
        \addlinespace % 增加行间距，让三线表更美观
        负样本擦除彻底性 & 94.5\% & 少数样本残留了标志性配色（如米老鼠的红裤子）。 \\
        \addlinespace
        时空结构对齐性 & 96.8\% & 极少数样本因分支后的随机性导致动作方向相反。 \\
        \midrule
        \textbf{综合合格率} & \textbf{92.6\%} & - \\
        \bottomrule
    \end{tabularx}
\end{table}

如表~\ref{tab:dataset_evaluation}所示，经自动化清洗后的数据集在综合合格率上达到了 92.6\%。这一结果有力地证明了 VCE-24K 数据集并非由模型随机生成的噪声数据，而是经过严格质量控制的、具备高信噪比的标准化基准。
\section{本章小结}
